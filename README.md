...
|---|---|---|
| [`gates-registry-total`](https://github.com/sayam/verifiable-gates/blob/main/docs/checker-reference.md#gates-registry-total) | A CI job with no row in the gate index, a row nothing can fail, a test file no gate claims | `gates.yaml`, every workflow under `.github/workflows`, the test files under `tests_path` |
