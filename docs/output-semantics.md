...
| A `purge_paths` that covers every module under `src_path` — `["**"]`, `["app/**"]` | `NA`, with the count (`every Python module under app is in purge_paths (12 of 12)`) | Not a pass: nothing outside the purge modules was read. Not a finding either — a project may keep every module that deletes under one glob; the NA says so and judges nothing (bypass case B18, measured 2026-09-08). |
