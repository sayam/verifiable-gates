...
### `an-instruction-is-run-from-what-the-reader-gets`

**Practice:** An instruction a document will carry is run first, from the artefact the reader will have

**Born from:** L-0177 · 2026-09-05 · The README quickstart was verified from this checkout, whose editable install has files the wheel does not: `pip install verifiable-gates` then `python -m verifiable_gates.install .` answered "the bundle is incomplete … refusing to install", exit 1. Every transcript was real, every sentence was true here, and every one of them was false for a reader.
...
### `a-badge-is-somebody-elses-cache`

**Practice:** What a badge shows a reader is checked where the reader is, not at the badge's own source

**Born from:** L-0197 · 2026-09-05 · An hour after a release was published the repository page still showed the previous version's badge. The image host had the new one; the copy served through the host's own image proxy was old, on a cache miss, and nothing in the post-release check had ever asked what a reader is served rather than what the source says.

**Held by:** reading this line — nothing here refuses it for you

**Apply:** After every release, fetch each badge through whatever proxy the page a reader opens puts in front of it, and record what it says with its cache headers. Know which badges should move and which are static, and know each one's window. Inside the window the honest report names both answers — the source says the new version, the proxy is still serving the old one, its window is N hours. **After** the window has passed, a stale value is a finding and not a wait: it means the badge's own source is wrong.
