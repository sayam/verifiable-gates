...
this bundle also carries the working: 14 practices, each with the lesson behind it and the pull requests it held on — off here; read `python3 tools/gates_doctor.py --working`, turn on with `install <dest> --working`
...
- **โปรเจกต์ที่ทุกข้อเป็น `NA` ออก 0 นั่นคือโปรเจกต์ที่ยังไม่ได้วัด** ไม่ใช่โปรเจกต์ที่ผ่าน
  (`DECISIONS.md` `doctor-all-na-exits-zero`) · manifest ที่ไม่มีด่าน scan เลยเป็นคนละเรื่อง:
  ไม่มีอะไรรัน รันนั้นไม่มีคำตัดสิน — ออก 2
- พาธที่ `scaffold.json` *ตั้งชื่อไว้* แต่โปรเจกต์ไม่มี เป็น finding ไม่ใช่ `NA` — คอนฟิกที่พังคือของเสีย ไม่ใช่ของที่ไม่มี
- **บันเดิลที่ record การติดตั้งรับรองไม่ได้ ไม่มีคำตัดสิน** scanner ที่ถูกแก้หรือหายไป หรือไม่มี
  `tools/installed.json` จะถูกบอกไว้เหนือบรรทัดด่านแรก และรันออก 2 ไม่ว่า scan จะพิมพ์อะไร ·
  `--installed` ให้บัญชีเต็ม · doctor ที่ถูกแก้เองจับจากในต้นไม้ไม่ได้
...
ไม่มีไฟล์ไหนที่ bundle ติดตั้งติดต่อออกนอกเครื่องนี้ (socket ตัวเดียวที่มีถามแค่ว่ามีบริการฟังอยู่ที่ `127.0.0.1` หรือไม่) · `python3 tools/gates_doctor.py --rules` พิมพ์รายการนี้จากบันเดิลที่ติดตั้งอยู่
พร้อมเหตุการณ์ที่ให้กำเนิดแต่ละข้อ

| id ของกฎ | จับอะไร | อ่านอะไร |
|---|---|---|
| [`gates-registry-total`](https://github.com/sayam/verifiable-gates/blob/main/docs/checker-reference.md#gates-registry-total) | job ใน CI ที่ไม่มีแถวในทะเบียน · แถวที่ไม่มีอะไรทำให้ล้มได้ · ไฟล์เทสต์ที่ไม่มี gate ไหนอ้าง | `gates.yaml` · ทุก workflow ใต้ `.github/workflows` · ไฟล์เทสต์ใต้ `tests_path` |
...
- คำศัพท์ของ `kind` `severity` `layer` และ `pillar` เป็นเซตปิด — และ severity คือสิ่งที่มีคนอ่าน:
  `blocking` คือทุก gate บนการรัน (สแกนแดงไม่ว่าแถวจะเขียนอะไร) `watched` คือคำสัญญา `watched_by`
  ที่ red-streak census วัด และไม่มีคำที่สาม เพราะป้ายที่ไม่มีการรันใดอ่านคือความผ่อนปรนที่ไม่มีใครส่งมอบ
